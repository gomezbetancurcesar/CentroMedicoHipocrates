...
namespace CentroMedicoHipocrates
{
    public partial class ListadoRecepcionistas : Form
    {
        ...
        {
            GridOperadores.CellClick += new DataGridViewCellEventHandler(this.OnCellClick);
            List<Usuario> usuarios = new Usuario().buscarPorRol("Recepcion");
            GridOperadores.Rows.Clear();
            foreach (var administrador in usuarios)
            {
                int index = GridOperadores.Rows.Add();
                DataGridViewRow fila = GridOperadores.Rows[index];
        ...
        private void OnCellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (GridOperadores.SelectedRows.Count > 0)
            {
                DataGridViewRow row = GridOperadores.SelectedRows[0];
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            if (this.validarDatosPersona())
            {
                ...
                {
                    //Rol de Recepcion u Operador
                    usuario.ioUsuarioRoles = new UsuarioRoles();
                    usuario.ioUsuarioRoles.ioRolId = 3;
